...
프로젝트/사용자 **생성·삭제 신청 → 승인 → 실제 리소스 반영 → 결과 통보**의 업무 효율을 높이기 위해 자원 생성 회수 LifeCycle을 설계했습니다. 아래는 그 아키텍처입니다.


<img width="1000" alt="image" src="https://github.com/user-attachments/assets/34cc0123-c3a5-4ce0-a294-73d5795cfede" />


사용자는 플랫폼에서 구글폼을 작성하는 것 외에 아무것도 하지 않고, 관리자는 Slack에서 버튼 하나만 누릅니다. 그 사이의 검증/호출/통보는 전부 파이프라인이 처리합니다.

| # | 단계 | 처리 내용 |
|---|---|---|
| 1 | **Google Form 신청** | 프로젝트/유저 생성·삭제 4종 신청서 접수 |
| 2 | **Apps Script 유효성 검사** | 스프레드시트를 조회하며 12개 항목 검증 후, 검사 결과를 시트에 기록 |
| 3 | **Slack Webhook 전송** | 정제된 신청 내역을 관리자 채널로 전송. 검사를 통과하면 승인/거절 버튼이 함께 붙고, 탈락하면 신청 불가 메시지만 전송 |
| 4 | **관리자 승인** | 버튼 클릭 시 `action_id`, `row`, `type`이 담긴 payload가 Apps Script로 반환 |
| 5 | **FastAPI 호출** | Apps Script가 신청 폼 종류에 맞는 엔드포인트를 호출, HAProxy VIP가 3-node FastAPI로 로드밸런싱 |
| 6 | **Keystone 반영** | FastAPI가 OpenStack SDK로 Keystone API를 호출해 프로젝트/사용자 생성·삭제 수행 |
| 7 | **Gmail 결과 통보** | 승인·거절·검증 탈락 등 **모든 결과**에 대해 신청자에게 이메일 자동 발송. 생성 건은 초기 계정 정보를 함께 전달 |
